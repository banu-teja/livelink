...
from livelink.guardrails import run_guardrails
from livelink.session_config import SessionConfig
from livelink.streaming import AudioDelta, StreamInterrupted, TextDelta, TurnComplete
from livelink.tools import ToolCall, ToolContext
from livelink.transport import AudioInput, ControlInput, TextInput, Transport
from livelink.types import ConversationTurn, LiveChunk
...
class RealtimeSession:
    ...
    Supervised usage::
        ...
        from livelink import SessionConfig
    ...
    def _init_supervision(self) -> None:
        ...
        from livelink.supervision.hitl import ApprovalGate, InputManager
    ...
    async def _connect(self) -> None:
        ...
        from livelink.registry import resolve_model
    ...
    async def _handle_tool_calls(self, calls: tuple[Any, ...]) -> None:
        ...
        from livelink.types import ToolResponse
        ...
        from livelink.handoff import is_handoff_result
        ...
        from livelink.delegation import is_delegation_result, parse_delegation_result
    ...
    async def _check_tool_approvals(
        ...
        from livelink.supervision.hitl import InputTimeoutError
        from livelink.tools import ToolResult
    ...
    async def _emit_approval_requested(self, tc: ToolCall) -> None:
        if not self._event_bus:
            return
        from livelink.supervision.events import ToolApprovalRequested
    ...
    async def _emit_approval_resolved(self, tc: ToolCall, approved: bool) -> None:
        if not self._event_bus:
            return
        from livelink.supervision.events import ToolApprovalResolved
    ...
    async def _start_delegation(self, backend_name: str, context: str) -> None:
        ...
        from livelink.delegation import DelegationHandle, DelegationState
        from livelink.memory import OperationalMemory
        from livelink.relay import SupervisionRelay
        from livelink.supervision import CancellationToken, EventBus, InputManager, supervise
    ...
    def _register_for_supervision(self) -> None:
        ...
        if self._config.supervision:
            from livelink.supervise import register_session
    ...
    def _unregister_from_supervision(self) -> None:
        ...
        if self._config.supervision:
            from livelink.supervise import unregister_session
    ...
    async def _emit_session_started(self) -> None:
        if not self._event_bus:
            return
        from livelink.supervision.events import SessionStarted
    ...
    async def _emit_session_ended(self) -> None:
        if not self._event_bus:
            return
        from livelink.supervision.events import SessionEnded
    ...
    async def _emit_turn_ended(self) -> None:
        if not self._event_bus:
            return
        from livelink.supervision.events import TurnEnded
    ...
    async def _emit_tool_started(self, tc: ToolCall) -> None:
        if not self._event_bus:
            return
        from livelink.supervision.events import ToolExecutionStarted
    ...
    async def _emit_tool_completed(self, tc: ToolCall, result: str, duration_ms: float) -> None:
        if not self._event_bus:
            return
        from livelink.supervision.events import ToolExecutionCompleted
    ...
    async def _emit_tool_failed(self, tc: ToolCall, error: str) -> None:
        if not self._event_bus:
            return
        from livelink.supervision.events import ToolExecutionFailed
